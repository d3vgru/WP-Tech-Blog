						...
						if /* Configd? */($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["pro_ccbill_client_id"])
							{
								...
								if(!($last = get_transient("s2m_".md5("s2member_pro_ccbill_last_datalink"))) || $last < (time() - 86400))
									{
										$start = /* Don't let $start time be less than 1 day + 12 hours ago. */ ($last && $last >= (time() - (86400 + 43200))) ? $last : time() - 86400;
										$end = $last = /* Sets $end time to exactly 1 day later. This also sets $last time; which is recorded later in this routine. */ ($start + 86400);
										/**/
