...
Author URI: http://www.inlet.nl
    ...
    Copyright 2011 Patrick Brouwer  (email : patrick@inlet.nl)
...
*		<?php endwhile; ?>
*
* Or with custom query_posts arguments (@see http://codex.wordpress.org/Function_Reference/query_posts):
*
...
/**
* Check if the current post is a portfolio item.
* @return bool
*/
function is_portfolio_single() {
	global $post;
	if ($post === null) return false;
	return (is_single($post) && $post->post_type == 'portfolio');
}

/**
* Check if the current page is portfolio taxonomy
* @return bool
*/
function is_portfolio() {
	global $post;
	if ($post === null) return false;
	return (!is_single() && get_post_type() == 'portfolio' && !is_search());
}
