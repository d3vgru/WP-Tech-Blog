...
jQuery(document).ready(function(){
    window.spAddContainer = jQuery("div#portfolio-media-add");
});

/**
 * Add Youtube ID
 */
function sp_add_youtube() {
    window.spAddContainer.slideUp('fast', function () {
        window.spAddContainer.html(get_add_html('Youtube ID', 'Youtube', 'textinput')).slideDown('fast', function () {
 ...
 * Add Html Snippet
 */
function sp_add_snippet() {
    window.spAddContainer.slideUp('fast', function () {
        window.spAddContainer.html(get_add_html('Snippet', 'Snippet', 'textarea')).slideDown('fast', function () {
            jQuery(this).find("#add-field").focus();
        });
    });
}
 ...
 * Add Text Paragraph
 */
function sp_add_text() {
    window.spAddContainer.slideUp('fast', function () {
        window.spAddContainer.html(get_add_html('Text', 'Text', 'textarea')).slideDown('fast', function () {
            jQuery(this).find("#add-field").focus();
        });
    });
}
 ...
 * Add an Image
 */
function sp_add_image() {
    jQuery('html').addClass('Image');
    var post_id = jQuery('#post_ID').val();
    post_id |= 0;

    tb_show('', 'media-upload.php?post_id=' + post_id + '&simpleportfolio_media=true&type=image&TB_iframe=true');
    return false;
}
    ...
    window.spAddContainer.slideUp('fast', function () {
        ...
        jQuery.ajax({
            ...
            success:function (data) {
                ...
                first.hide().slideDown('normal').find('textarea').attr('value', value);
